...
function isNumber(id)
	if id and type(id)=="number" then return true end
	return false
end
...
function getPersIdToId(pid)
	if not pid or type(pid)=="userdata" then return pid end
	if isRaid() then
...
function getIdToPersId(persid)
	if not persid or type(persid)=="number" then return persid end
	if isRaid() then
		local members=raid.GetMembers()
		for i, g in pairs(members) do
			for j, m in pairs(g) do
				if m and (m.uniqueId and m.uniqueId==persid or m.persistentId and m.persistentId==persid) then return m.id end
			end
		end
	elseif isGroup() then
		local members=group.GetMembers()
	...
	elseif avatar.GetUniqueId and avatar.GetUniqueId()==persid or avatar.GetServerId and avatar.GetServerId()==persid then
		return avatar.GetId and avatar.GetId()
	end
	return persid
end
